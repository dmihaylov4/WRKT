...
enum AppLogger {
    ...
    nonisolated static let workout = Logger(subsystem: subsystem, category: "Workout")

    /// HealthKit integration
    nonisolated static let health = Logger(subsystem: subsystem, category: "Health")

    /// Rewards and gamification
    nonisolated static let rewards = Logger(subsystem: subsystem, category: "Rewards")
    ...
    /// Battle system
    nonisolated static let battles = Logger(subsystem: subsystem, category: "Battles")

    /// Challenge system
    nonisolated static let challenges = Logger(subsystem: subsystem, category: "Challenges")

    /// Notification system
    nonisolated static let notifications = Logger(subsystem: subsystem, category: "Notifications")

    /// Social features (posts, comments, friends)
    nonisolated static let social = Logger(subsystem: subsystem, category: "Social")

    /// Virtual run feature
    nonisolated static let virtualRun = Logger(subsystem: subsystem, category: "VirtualRun")
    ...
    nonisolated static func debug(_ message: String, category: Logger? = nil, file: String = #file, function: String = #function, line: Int = #line) {
        #if DEBUG
    ...
    nonisolated static func info(_ message: String, category: Logger? = nil) {
        let logger = category ?? app
        logger.info("\(message)")
    }

    /// Log warnings that don't prevent functionality
    nonisolated static func warning(_ message: String, category: Logger? = nil) {
        let logger = category ?? app
        logger.warning("⚠️ \(message)")
    }

    /// Log errors that affect functionality
    nonisolated static func error(_ message: String, error: Error? = nil, category: Logger? = nil) {
        let logger = category ?? app
        if let error = error {
    ...
    nonisolated static func critical(_ message: String, error: Error? = nil, category: Logger? = nil) {
        let logger = category ?? app
        if let error = error {
    ...
    nonisolated static func success(_ message: String, category: Logger? = nil) {
        let logger = category ?? app
        logger.info("✅ \(message)")
    }
}
