...
extension Exercise {
    ...
    init(from dto: ExcelExerciseDTO) {
        ...
        // Infer tracking mode from loadPosition if not explicitly set
        if let explicitMode = dto.trackingMode?.trimmedOrNil {
            self.trackingMode = explicitMode
        } else if let loadPosition = dto.loadPosition?.trimmedOrNil?.lowercased(),
                  loadPosition == "no load" || loadPosition.contains("bodyweight") || loadPosition.contains("body weight") {
            // Bodyweight exercises (no load)
            self.trackingMode = "bodyweight"
        } else {
            // Default to weighted for everything else
            self.trackingMode = "weighted"
        }
