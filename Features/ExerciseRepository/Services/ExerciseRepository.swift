...
final class ExerciseRepository: ObservableObject {
    ...
    @Published public private(set) var didLoadFull = false
    ...
    func loadFirstPage(with filters: ExerciseFilters = ExerciseFilters()) async {
        ...
        // Merge custom exercises that match filters (shown at the top of results)
        let matchingCustom = customStore.customExercises.filter { ex in
            matchesFilters(ex, filters: filters)
        }

        // Custom exercises appear first, then catalog exercises
        self.exercises = matchingCustom + page
        self.totalExerciseCount = total + matchingCustom.count
        self.hasMorePages = page.count >= cache.pageSize

        AppLogger.debug("Loaded first page: \(page.count) exercises + \(matchingCustom.count) custom (total: \(totalExerciseCount))", category: AppLogger.app)
    }

    /// Check if exercise matches current filters
    private func matchesFilters(_ ex: Exercise, filters: ExerciseFilters) -> Bool {
        // Muscle group filter
