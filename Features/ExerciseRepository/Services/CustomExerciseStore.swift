...
final class CustomExerciseStore: ObservableObject {
    ...
    init() {
        ...
        if let documentsDir = fileManager.urls(for: .documentDirectory, in: .userDomainMask).first {
            ...
            AppLogger.warning("Documents directory unavailable, using temp directory", category: AppLogger.storage)
            baseDir = fileManager.temporaryDirectory
        }

        let storageDir = baseDir.appendingPathComponent("WRKT_Storage", isDirectory: true)
