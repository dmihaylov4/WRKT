    ...
    var body: some View {
        ScrollViewReader { proxy in
            ...
            .onChange(of: selected) { _, newValue in
                // Smoothly scroll to show selected chip (use leading anchor to keep alignment)
...
struct SelectChip: View {
    ...
    private var fillColor: Color {
        selected ? DS.Palette.marone.opacity(0.15) : DS.Semantic.surface50.opacity(0.3)
    }

    private var borderColor: Color {
        selected ? DS.Palette.marone.opacity(0.5) : DS.Semantic.border.opacity(0.3)
    }
