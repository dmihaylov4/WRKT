...
final class RewardsEngine: ObservableObject {
    ...
    internal var rules: RewardsRules = .empty  // internal so extensions can access
...
extension RewardsEngine {
    ...
    func resetAll() {
        ...
        if let progress = progress {
            AppLogger.info("Resetting RewardProgress: Before - weeklyStreak=\(progress.weeklyGoalStreakCurrent), superStreak=\(progress.weeklySuperStreakCurrent)", category: AppLogger.rewards)
            ...
            progress.weeklySuperStreakCurrent = 0
            progress.weeklySuperStreakLongest = 0
            progress.lastWeekSuperStreakMet = nil

            // Reset data cutoff date - only workouts after NOW will count
            progress.dataCutoffDate = Date.now

            AppLogger.info("Resetting RewardProgress: After - weeklyStreak=\(progress.weeklyGoalStreakCurrent), superStreak=\(progress.weeklySuperStreakCurrent), cutoffDate=\(progress.dataCutoffDate)", category: AppLogger.rewards)
        }
...
extension RewardsEngine {
    ...
    func checkWeeklyGoalStreak(
        ...
        if streakResult.didIncrease || streakResult.hitMilestone {
            ...
            // Apply milestone XP only if this milestone hasn't been awarded yet
            // This prevents duplicate XP awards when the app is reopened
            var xpAwarded = 0
            if streakResult.milestoneXP > 0 {
                let lastAwarded = progress?.lastAwardedMilestoneLevel ?? 0
                // Only award if this streak level is higher than the last awarded milestone
                if streakResult.new > lastAwarded {
                    applyWalletAndLevel(deltaXP: streakResult.milestoneXP, deltaCoins: 0)
                    progress?.lastAwardedMilestoneLevel = streakResult.new
                    xpAwarded = streakResult.milestoneXP
                    AppLogger.info("Awarded \(xpAwarded) XP for \(streakResult.new)-week milestone (previous: \(lastAwarded))", category: AppLogger.rewards)
                } else {
                    AppLogger.debug("Skipping milestone XP - already awarded for level \(lastAwarded), current: \(streakResult.new)", category: AppLogger.rewards)
                }
            }
            ...
            return (true, streakResult.new, xpAwarded)
        }
    ...
    /// Validate and reset weekly streak if necessary
    /// Returns true if streak was reset
    @discardableResult
    func validateWeeklyStreakOnAppear(store: WorkoutStoreV2) -> Bool {
        return validateWeeklyGoalStreak(store: store)
    }
}
