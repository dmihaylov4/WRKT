...
actor StatsAggregator {
    ...
    func computeMovingAverages(window: Int = 4) async {
        ...
        AppLogger.info("Moving averages: \(allWeeks.count) weeks, avg \(personalAvg.safeInt) volume", category: AppLogger.statistics)
    }
    ...
    func computeBalanceMetrics(for workouts: [CompletedWorkout], allWorkouts: [CompletedWorkout]? = nil) async {
        ...
        // Check if repo has exercises loaded
        let repoExerciseCount = await MainActor.run { repo.exercises.count }
        if repoExerciseCount == 0 {
            AppLogger.warning("ExerciseRepository has 0 exercises loaded, skipping balance metrics", category: AppLogger.statistics)
            return
        }
        AppLogger.info("Computing balance metrics with \(repoExerciseCount) exercises in repo", category: AppLogger.statistics)
        ...
        var totalExercisesProcessed = 0
        var totalExercisesNotFound = 0
        ...
        for weekStart in affectedWeeks {
            ...
            AppLogger.debug("Processing week \(weekKey): \(exerciseVolumes.count) exercise volumes", category: AppLogger.statistics)
            ...
            // PERFORMANCE FIX: Batch fetch all exercises before processing to avoid N MainActor switches
            let exerciseIDs = exerciseVolumes.map { $0.exerciseID }
            let exercisesDict: [String: Exercise] = await MainActor.run {
                Dictionary(uniqueKeysWithValues: exerciseIDs.compactMap { id -> (String, Exercise)? in
            ...
            for evs in exerciseVolumes {
                totalExercisesProcessed += 1

                // Fetch exercise from pre-loaded dictionary (no MainActor switch)
                guard let ex = exercisesDict[evs.exerciseID] else {
                    totalExercisesNotFound += 1
                    AppLogger.warning("Exercise not found in repo: \(evs.exerciseID) (volume: \(evs.volume.safeInt))", category: AppLogger.statistics)
                    continue
                }
            ...
            if let ppb = try? fetchOrCreatePushPullBalance(key: weekKey, weekStart: weekStart, in: ctx) {
                ...
                AppLogger.info("Week \(weekKey): Push=\(Int(pushVol)), Pull=\(Int(pullVol)), Ratio=\(String(format: "%.2f", ppb.ratio))", category: AppLogger.statistics)
            }
        ...
        // Summary logging
        if totalExercisesNotFound > 0 {
            AppLogger.warning("Balance metrics complete: \(totalExercisesProcessed) processed, \(totalExercisesNotFound) not found in repo (\((Double(totalExercisesNotFound)/Double(max(1,totalExercisesProcessed))*100).safeInt)%)", category: AppLogger.statistics)
        } else {
            AppLogger.info("Balance metrics complete: \(affectedWeeks.count) weeks, \(totalExercisesProcessed) exercises processed", category: AppLogger.statistics)
        }
    }
    ...
    private func computeMuscleFrequency(repo: ExerciseRepository, workouts: [CompletedWorkout], in ctx: ModelContext) async {
        ...
        for workout in recentWorkouts {
            ...
            for entry in workout.entries {
                ...
                if !muscleGroups.isEmpty {
                    AppLogger.debug("  Exercise '\(ex.name)' → muscle groups: \(muscleGroups.joined(separator: ", "))", category: AppLogger.statistics)
                }
        ...
        // Save to database
        AppLogger.info("MuscleFrequency: Saving \(muscleStats.count) muscle groups to database", category: AppLogger.statistics)
