...
struct MuscleIndex: Decodable {
    ...
    func ids(forClassTokens tokens: [String]) -> [String] {
        ...
        for t in tokens {
            ...
            if let f = front.classToIds[normalized] { out.formUnion(f) }
            if let b = back .classToIds[normalized] { out.formUnion(b) }
        }
        return Array(out)
    }
...
private extension MuscleIndex {
    ...
    static func parseSVG(named name: String) -> [String: [String]] {
        ...
        // Updated patterns to handle optional spaces around = in attributes
        let p1 = #"<g\b[^>]*?\bid\s*=\s*"([^"]+)"[^>]*?\bclass\s*=\s*"([^"]+)"[^>]*?>"#
        let p2 = #"<g\b[^>]*?\bclass\s*=\s*"([^"]+)"[^>]*?\bid\s*=\s*"([^"]+)"[^>]*?>"#
        ...
        func ingest(_ re: NSRegularExpression, _ idIdx: Int, _ classIdx: Int) {
            re.matches(in: raw, range: NSRange(location: 0, length: ns.length)).forEach { m in
                ...
                let idNormalized = id.lowercased().replacingOccurrences(of: "_", with: "-")
                tokens.append(idNormalized) // Add full ID as a token
                tokens += idNormalized.split(separator: "-").map(String.init) // Add split parts
