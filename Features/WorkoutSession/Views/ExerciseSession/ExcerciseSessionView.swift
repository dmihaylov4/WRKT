...
struct ExerciseSessionView: View {
    ...
    private var canShowSupersetToggle: Bool {
        guard let entryID = viewModel.currentEntryID else { return false }
        return store.canSuperset(entryID: entryID)
    }

    private var isInSuperset: Bool {
        guard let entryID = viewModel.currentEntryID else { return false }
        return store.isInSuperset(entryID: entryID)
    }
    ...
    var body: some View {
        GeometryReader { geometry in
            ZStack {
                ...
                // Workout toast overlay for in-session notifications
                WorkoutToastOverlay()
                    .zIndex(998)

                // Rest timer completion toast - needs to be above all other overlays
    ...
    private func handleOnAppear(geometry: GeometryProxy) {
        ...
        // Call ViewModel's onAppear to set up notification listeners
        viewModel.onAppear()

        preloadExistingIfNeeded()
        

        // If sets are still default (1 set with 0 weight), prefill from history with progression
        let hasDefaultSet = viewModel.sets.count == 1 && viewModel.sets[0].weight == 0
       
        if hasDefaultSet && !viewModel.didPrefillFromHistory {
           
            viewModel.prefillFromHistory()
            
            if let firstSet = viewModel.sets.first {
        ...
        if viewModel.activeSetIndex < viewModel.sets.count {
            let activeSet = viewModel.sets[viewModel.activeSetIndex]
            if !activeSet.isCompleted && activeSet.startTime == nil {
                viewModel.sets[viewModel.activeSetIndex].startTime = Date()
            }
        }
    ...
    private var modernHeader: some View {
            ...
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    HStack(spacing: 8) {
                        ...
                        // Progression indicator (shown when smart progression is applied)
                        if let progression = viewModel.appliedProgression {
                            let displayWeight = unit == .kg ? progression.suggestedWeight : progression.suggestedWeight * 2.20462
        ...
        .sheet(isPresented: $viewModel.showInfo) {
            exerciseInfoSheet
                .onAppear {
                    selectedInfoTab = .info // Reset to info tab when sheet opens
                }
        }
    }
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    if !guideMeta.equipment.isEmpty {
                        HStack(spacing: 12) {
                            ...
                            if !guideMeta.difficulty.isEmpty {
                                Text("•")
                                    .foregroundStyle(Theme.secondary)
                                HStack(spacing: 6) {
    ...
    private var infoTabContent: some View {
        VStack(spacing: 24) {
            ...
            if let media = repo.media(for: exercise),
               let videoURL = media.youtube {
                VStack(alignment: .leading, spacing: 12) {
                    SectionHeader(title: "Tutorial", icon: "play.rectangle.fill")
                        .padding(.horizontal, 16)
            ...
            // Progression Section (if applied)
            if let progression = viewModel.appliedProgression {
                VStack(alignment: .leading, spacing: 12) {
                    SectionHeader(title: "Your Progression", icon: "arrow.up.right.circle.fill")
                        .padding(.horizontal, 16)

                    ProgressionCard(progression: progression, unit: unit)
                        .padding(.horizontal, 16)
                }
            }
            ...
            VStack(alignment: .leading, spacing: 12) {
                SectionHeader(title: "Technique Tips", icon: "lightbulb.fill")
                    .padding(.horizontal, 16)
    ...
    private var statsTabContent: some View {
        ExerciseStatisticsView(
            ...
            trackingMode: TrackingMode(rawValue: exercise.trackingMode ?? "weighted") ?? .weighted
        )
    ...
    private var contentList: some View {
        List {
            SetsSection(
                ...
                canShowSupersetToggle: canShowSupersetToggle,
                isInSuperset: isInSuperset,
                onToggleSuperset: {
                    if let entryID = viewModel.currentEntryID {
                        store.toggleSupersetWithPrevious(entryID: entryID)
    ...
    private struct SetsSection: View {
        ...
        let canShowSupersetToggle: Bool
            ...
            Section {
                ...
                ForEach(Array(sets.indices), id: \.self) { i in
                    ...
                    AdaptiveSetRow(
                        ...
                        onActivate: {
                            activeSetIndex = i
                            // Initialize timing when set becomes active
                            if !sets[i].isCompleted && sets[i].startTime == nil {
                                sets[i].startTime = Date()
                            }
                        },
                        onLogSet: {
                            onLogSet(i)
    ...
    private func preloadExistingIfNeeded(force: Bool = false) {
        ...
        guard let id = viewModel.currentEntryID else {
        ...
        guard force || !viewModel.didPreloadExisting else {
        ...
        if let entry = store.currentWorkout?.entries.first(where: { $0.id == id }) {
            ...
            if !entry.sets.isEmpty {
                ...
                AppLogger.warning("Entry has empty sets, using defaults", category: AppLogger.workout)
            }
        } else {
            AppLogger.warning("No entry found in currentWorkout", category: AppLogger.workout)
        }
    ...
    private func logSet(at index: Int) {
        ...
        // 1. Capture timing data and mark set as completed
        let now = Date()

        // If startTime was never set (shouldn't happen, but handle gracefully)
        if viewModel.sets[index].startTime == nil {
            // Estimate start time was ~30 seconds ago (reasonable for a typical set)
            viewModel.sets[index].startTime = now.addingTimeInterval(-30)
        }

        viewModel.sets[index].completionTime = now
        viewModel.sets[index].isCompleted = true

        // Store rest timer duration that will be used (for later capture)
        let prefs = RestTimerPreferences.shared
        if prefs.isEnabled {
            let duration = prefs.restDuration(for: exercise)
            viewModel.sets[index].restAfterSeconds = Int(duration)
        }
        ...
        if index < viewModel.sets.count - 1 {
            ...
            // Note: startTime and actualRestSeconds will be set when rest timer completes/skips
        }
    ...
    private func generateNewSetAfterRest() {
        ...
        // Calculate ACTUAL rest taken from the last completed set (do this FIRST, before any early returns)
        if let lastCompletedIndex = viewModel.sets.lastIndex(where: { $0.isCompleted }),
           let previousCompletionTime = viewModel.sets[lastCompletedIndex].completionTime {
            let actualRest = (restEndTime.timeIntervalSince(previousCompletionTime)).safeInt
            viewModel.sets[lastCompletedIndex].actualRestSeconds = actualRest
            needsSave = true
        }
        ...
        if let lastSet = viewModel.sets.last, !lastSet.isCompleted {
            ...
            // Set start time for the activated set
            if viewModel.sets[viewModel.activeSetIndex].startTime == nil {
                viewModel.sets[viewModel.activeSetIndex].startTime = restEndTime
                needsSave = true
            }

            // Save the updated actualRestSeconds
        ...
        let completedSetsCount = viewModel.sets.filter { $0.isCompleted }.count
        if completedSetsCount >= 4 {
            // Save the updated actualRestSeconds even though we're not adding a new set
            if needsSave, let entryID = viewModel.currentEntryID {
                store.updateEntrySetsAndActiveIndex(entryID: entryID, sets: viewModel.sets, activeSetIndex: viewModel.activeSetIndex)
            }
        ...
        let newSet = SetInput(
            ...
            startTime: restEndTime  // Initialize timing for the new set
        )

        viewModel.sets.append(newSet)
        viewModel.activeSetIndex = viewModel.sets.count - 1

        // Auto-save the new set to the workout (includes actualRestSeconds update)
        if let entryID = viewModel.currentEntryID {
            store.updateEntrySetsAndActiveIndex(entryID: entryID, sets: viewModel.sets, activeSetIndex: viewModel.activeSetIndex)
        }
    ...
    private func startRestTimerIfEnabled() {
        ...
        // Check if we're in a superset and should skip the rest timer
        if let entryID = viewModel.currentEntryID,
           let workout = store.currentWorkout,
           let entry = workout.entries.first(where: { $0.id == entryID }),
           entry.supersetGroupID != nil {
            let completedCount = entry.sets.filter { $0.isCompleted }.count
            let isLastInRound = workout.isLastInSupersetRound(entryID: entryID, completedSetCount: completedCount)

            // Skip rest timer between superset exercises (only rest after completing a full round)
            if !isLastInRound {
                AppLogger.debug("Skipping rest timer - superset exercise, not end of round", category: AppLogger.workout)
                return
            }
        }
        ...
        // Always start with full rest duration when completing a set
        // Don't reuse remaining time from previous timer
        let duration = prefs.restDuration(for: exercise)
...
private struct ProgressionCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            // Show the change — reps for bodyweight/rep progression, weight otherwise
            if progression.type == .increaseReps && progression.previousWeight == 0 {
                // Bodyweight exercise: show rep progression
                HStack(spacing: 12) {
                ...
                // Weighted exercise: show weight progression
                HStack(spacing: 12) {
                    VStack(alignment: .leading, spacing: 4) {
                        Text("Previous")
                            .font(.caption)
        ...
        .padding(16)
        .background(ExerciseSessionTheme.surface)
        .clipShape(ChamferedRectangle(.medium))
        .overlay(ChamferedRectangle(.medium).stroke(ExerciseSessionTheme.border, lineWidth: 1))
    }
}
