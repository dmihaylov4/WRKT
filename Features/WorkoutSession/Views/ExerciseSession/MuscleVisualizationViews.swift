...
private enum SVGMuscleHeatmap {
    static let primaryName   = "red"
    static let secondaryName = "orange"
    static let tertiaryName  = "yellow"
    static let primaryAlpha:   Double = 0.85
    static let secondaryAlpha: Double = 0.60
    static let tertiaryAlpha:  Double = 0.40
}
...
struct SVGHumanBodyView: View {
    ...
    var body: some View {
        ...
        .id("\(side)-\(focus)-\(highlightKey)") // Force recreation when any key parameter changes
    }
...
// MARK: - SVG Human Body View Inner

private struct SVGHumanBodyViewInner: View {
    let side: SVGHumanBodyView.Side
    let focus: SVGHumanBodyView.Focus
        ...
        Group {
            if let url = Bundle.main.url(forResource: side == .front ? "torso" : "torso_back",
                ...
                svg
                    ...
                    .mask(maskForFocus())
                    .onAppear {
                        // Delay to ensure SVG is fully loaded (same pattern as ProfileView)
                        DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
                            applyHighlights(into: svg)
                        }
                    }
            } else {
    ...
    private func applyHighlights(into root: SVGView) {
        ...
        color(ids: Array(tIDs), in: root, colorName: SVGMuscleHeatmap.tertiaryName, opacity: SVGMuscleHeatmap.tertiaryAlpha)
        color(ids: Array(sIDs), in: root, colorName: SVGMuscleHeatmap.secondaryName, opacity: SVGMuscleHeatmap.secondaryAlpha)
        color(ids: Array(pIDs), in: root, colorName: SVGMuscleHeatmap.primaryName,   opacity: SVGMuscleHeatmap.primaryAlpha)
    }
...
enum MuscleLexicon {
    static func tokens(for raw: String) -> [String] {
        ...
        case "deltoid", "deltoids", "deltoid-exterior", "deltoid-lateral", "lateral deltoid", "lateral deltoids", "middle deltoid", "middle deltoids", "side delts", "side delt", "lateral-deltoid", "lateral-deltoids":
            out.formUnion(["deltoid","deltoid-lateral","lateral-deltoid","deltoid-exterior"])
