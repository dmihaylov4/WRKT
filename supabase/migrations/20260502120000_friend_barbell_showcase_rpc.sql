-- Friend-safe barbell showcase read.
-- Returns only public room presentation fields and racked plate rendering fields.
...
begin
    ...
    select
        ...
        p.tier_id,
        p.weight_kg,
        case when s.show_plate_engravings then p.engraving_text else '' end,
        p.earned_by_event,
        p.lift_type_id,
        p.current_tier,
        p.chalk_use_count,
...
comment on function public.get_friend_barbell_showcase(uuid) is
    'Friend-safe barbell room read. Does not expose workout IDs, biography events, or private plate collection rows.';
